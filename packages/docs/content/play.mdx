...
export const initialDoc = `const {
  ErrorMessage, TextMessage, ErrorItem, HelpItem,
  parse, parseInto, validate, config, req, tf, fg, bg, ul, style, fg8, bg8, ul8,
} = tsargp;
...
<Code callbacks={callbacks} initialDoc={initialDoc} initialSel={[163]} />
