...
The parser can emit a list of words or even fully featured suggestion objects for the purpose of [tab-completion], which is performed by the terminal's native completion engine, if one is available. The only configuration required is to register the final executable in the end-user's shell.
