...
From a non-functional standpoint, we are yet to perform some benchmarks, but we can attest that the codebase contains almost 700 handcrafted test cases and over 1.2k `expect(){:ts}` calls, covering virtually every line of library code.
