...
| 34KB minified        | Subcommands            | Option grouping/hiding  |
...
By relying on [TypeScript], it guarantees that the values resulting from argument parsing reflect the attributes specified in the option definitions. In particular, an option value will be a union literal if you declare a parameter mapping in its definition. Even JSDoc comments are preserved in [IntelliSense]!
...
The parser can emit a list of words or even fully featured suggestion objects for the purpose of [tab-completion], which is performed by the native completion engine (if one is available). The only configuration required is to register the final executable in the end-user's terminal.
