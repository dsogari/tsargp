...
| 32KB minified        | Nested commands        | Custom phrasing         |
| ESM-native           | Asynchronous callbacks | Option grouping/hiding  |
| Online documentation | GNU's _short-options_  | Help formats/sections   |
...
```ts copy filename="<your_cli_name>.ts" {7,8} /<your_cli_name>/
#!/usr/bin/env node
...
```ts copy filename="<your_cli_name>.spec.ts" /<your_cli_name>/
