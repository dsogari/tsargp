...
You can configure shell completion to use the main script as a source of completion words or suggestions. This is handled automatically by the library. You just need to register it with the completion builtins.
...
<Tabs items={['Bash', 'PowerShell', 'Zsh', 'Fish', 'Nushell', 'Fig']}>
  ...
  <Tabs.Tab>
    Support for [Fig] is limited because the upstream project is somewhat lacking. For instance, it got integrated into AWS and it does not currently support Linux or Windows. Still, you may be able to use it on MacOS with the provided examples. Look in the [autocomplete] folder for reference.
  </Tabs.Tab>
</Tabs>
