...
Another way to offer help is by defining a "help" [subcommand]. This method still uses the [help option] from the previous example, but increments it by providing a new syntax to obtain help for subcommands. For example, to present help for a subcommand called `cmd`, the following invocations would be equivalent:
...
To achieve this, we have to define a "helper" subcommand that accepts the name of a subcommand for which the actual help message should be produced. Let's start with the option definitions for the subcommands:
