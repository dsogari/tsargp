...
To achieve this, we have to define a [command option]. We can start with the definitions for the subcommands' options:
...
Keep in mind that, when word completion is in effect, the command's [parse callback] will _not_ be executed. Conversely, if it _is_ executed, you need not worry about the [`compIndex`] flag for the delegated `parse` call.
