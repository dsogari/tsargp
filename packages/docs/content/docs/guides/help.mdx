...
<Callout type="default">
  Since this guide was first written, the [enable nested command] attribute has been introduced. It
  supersedes help commands and offers a more convenient way to achieve what is described below.
  However, if you need [parameter choices] for the available command names, then what follows might
  still be relevant.
</Callout>
...
<Callout type="info">
  Keep in mind that, when word completion is in effect, the command's [parse callback] will _not_ be
  executed. Conversely, if it _is_ executed, you need not worry about the [`compIndex`] flag for the
...
[nested options callback]: ../library/options#nested-options--callback
