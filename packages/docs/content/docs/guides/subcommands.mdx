...
Inside the [parse callback], we perform a typecast to access the values parsed for the command. This is necessary because the library does not know the concrete type of our option values when it calls the callback (and the callback type cannot be defined in terms of a generic type parameter). So it must pass an _opaque_ reference to them.
