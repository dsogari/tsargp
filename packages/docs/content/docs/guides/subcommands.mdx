...
From the values received in the callback, we select the result from the next operation (if any), falling back to a default if this was the last operation. We then return the accumulated result so far. Except if, in the case of `sub` and `mult`, two arguments are provided before another recursive call, then the result of that call is ignored.
