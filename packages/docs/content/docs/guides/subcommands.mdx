...
From the values received in the callback, we select the result from the next operation, falling back to a default if this was the last operation. We then return the result accumulated so far. Except if, in the case of `sub` and `mult`, two arguments are provided before another recursive call, then the result of that call is ignored.
