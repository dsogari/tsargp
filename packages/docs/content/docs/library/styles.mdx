...
- `a` -- an array or tuple
- `o` -- an `object{:ts}`
- `v` -- an `unknown{:ts}` value (enclosed in angle brackets)
...
- `add` -- appends a ANSI string formatted from one of the [error phrases]
- `addCustom` -- appends a ANSI string formatted from a _custom_ phrase
...
- `createCustom` -- creates an error message formatted from a _custom_ phrase
...
| `versionFileNotFound`        | A version file could not be found when parsing the version option               | `'Could not locate a version JSON file at #0.'{:ts}`                                             | `#0` = the file URL                                                                           |
| `disallowedInlineParameter`  | An option is supplied with an inline parameter, despite being disallowed        | `'(Option\|Positional marker) #0 does not accept inline parameters.'{:ts}`                       | `#0` = the supplied option name or positional marker                                          |
