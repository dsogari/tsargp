...
| `disallowedInlineParameter`  | An option is supplied with an inline parameter, despite being disallowed        | `'Option name #0 does not accept inline parameters.'{:ts}`                                       | `#0` = the supplied option name                                                               |
...
| `marker`     | Whether the option accepts trailing arguments                            | `'Accepts trailing arguments preceded by #0.'{:ts}`                              | `#0` = the [trailing marker]     |
