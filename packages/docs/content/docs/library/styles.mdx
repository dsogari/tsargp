...
The ANSI string can be chained with another instance for the purpose of _multi-column_ wrapping. We call this mode "line-wise", because it allows the lines of each ANSI string to be wrapped independently of other instances. This feature is used by the [formatter] when rendering a [groups section] configured with non-responsive layout.
