...
> [!NOTE]
>
> Usually, client code does not need to worry about resetting attributes, except when using [inline styles].
...
- `default` -- the default color (this is a resetting attribute)
- Standard colors:
  - `black` -- black
  - `red` -- red
  - `green` -- green
...
```ts twoslash
import { style, rgb, ul } from 'tsargp';
// ---cut---
...
- `a` -- an array or tuple
- `o` -- an `object{:ts}`
- `v` -- an `unknown{:ts}` value (enclosed in angle brackets, like the [parameter column])
...
Text can contain inline styles. This applies not only to phrases, but to text attributes as well (which the [formatter] splits into words before including in a [help message]).
...
> [!TIP]
>
> You can also use external styling packages for this purpose, such as the ubiquitous [chalk].
...
The ANSI string has an optional indentation level, i.e., the starting terminal column of each _new_ line of text.

When wrapping text to the desired width, the ANSI string will attempt to respect this setting, but if the desired width does not accommodate the length of the _largest_ word, it will disregard the indentation level.

> [!NOTE]
>
> The first line is only affected by this setting if the current cursor position is _less_ than the configured value.

#### Text alignment

The ANSI string has an optional flag that indicates whether the text should be aligned to the terminal's right boundary. This feature is used by the [formatter] in the [description column] of help messages.
...
The `AnsiMessage` class wraps a list of ANSI strings and is the type used for [help messages]. It provides a `wrap` method to get a plain string, accepting the following optional parameters:
...
The `JsonMessage` class represents a list of JavaScript objects and is the type used for [completion suggestions]. It produces a serialized JSON string, and should be printed with `console.log{:ts}` or equivalent.
...
> [!TIP]
>
> To preserve previous styles for a particular element, you can assign it an empty `Style{:ts}`. Or, to disable styling for your application as a whole (regardless of environment configuration), do the same for all elements.
...
| `invalidInlineConstraint`    | A variadic option declares an invalid inline constraint                      | `'Option #0 has invalid inline constraint.'{:ts}`                                                | `#0` = the option's key                                                                       |
...
| `link`       | The option's external resource hyperlink                                     | `'Refer to #0 for details.'{:ts}`                                                | `#0` = the [media hyperlink]     |
...
[formatter]: formatter
[help message]: formatter#help-message
