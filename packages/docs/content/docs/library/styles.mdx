...
> [!TIP]
>
> You can easily create ANSI string instances with the `ansi` tag function for tagged [template literals]. Just be aware that arguments inside the template will be treated as [placeholder] arguments.
...
Splitted text is always wrapped according to a configured _width_, i.e., the number of terminal columns by which to limit each line of text. When a width of zero or `NaN{:ts}` is provided, the ANSI string instance will by default avoid wrapping and instead suppress control sequences that would otherwise be used to render the text in a terminal.
...
[bug report]: https://github.com/dsogari/tsargp/issues/new?template=bug-report.yml
