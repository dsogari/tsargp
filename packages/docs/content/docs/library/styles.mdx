...
The `style` function accepts a list of styling attributes and creates a `Style` that can be used in any part of the API that expects this type. It can also be embedded directly in text attributes, such as:
...
When a width of zero or `undefined{:ts}` is provided (which may happen if the output is being redirected), the ANSI string will by default avoid wrapping and instead suppress some control sequences that would otherwise be used to render the text in a terminal.
...
This class provides an additional `wrap` method to get a traditional string. It accepts the following optional parameters:

- `width` - the desired terminal width (defaults to `0{:ts}`, i.e., avoid wrapping)
- `emitStyles` - whether styles should be emitted
- `emitSpaces` - whether spaces should be emitted instead of move sequences

The default value of the latter two parameters depend on a few environment variables:
...
It also offers two methods that used by the library:
...
Similarly to the warning message, it offers two static methods that used by the library:
