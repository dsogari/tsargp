...
When splitting text, the ANSI string instance will try its best to preserve inline styles. However, if you find some corner case that is not currently covered by our unit tests, please submit a [bug report].
...
When a width of zero or `undefined{:ts}` is provided (which may happen if the output is being redirected), the ANSI string instance will by default avoid wrapping and instead suppress control sequences that would otherwise be used to render the text in a terminal.
...
When wrapping text to the desired width, the instance will attempt to respect this setting, but if the desired width does not accommodate the length of the _largest_ word, it will disregard the indentation level.
...
> [!NOTE]
>
> The reason why an error may be raised is that we cannot split _words_, as they may have [inline styles] and those styles should not leak into subsequent columns.
