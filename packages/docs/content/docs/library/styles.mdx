...
The `AnsiString` class is an internal component that handles the construction of strings that can be printed on a terminal. It has methods to split, append and wrap strings, with or without control sequences, and is used by the library in different places:
...
- by both the [parser] and [validator], to produce error and warning messages based on configured [custom error phrases]
- by the [formatter], to assemble help items from configured [custom help phrases]
...
The `TextMessage` class represents a list of plain text lines with no wrapping. It is used for version and completion words. It produces a string delimited by line feeds, and should be printed with `console.log{:ts}` or equivalent.

### Message configuration

Messages can be configured via a `MessageConfig` object that contains the properties described below.

#### Message styles

The `styles` property specifies the styles of text elements in ANSI messages, and has the following optional properties:
...
The `connectives` property specifies connective words that are used to format some text elements that cannot be customized using phrases, such as option requirements, array element separators, and string quoting. It has the following optional properties, whose keys are enumerators from `ConnectiveWord`:
...
#### Custom error phrases

The `errorPhrases` property specifies custom phrases to use for error and warning messages. Its properties are optional and are indexed by the `ErrorItem` enumeration, which lists the kinds of errors and warnings that may be produced by both the parser and validator. They are summarized in the table below:
...
The `helpPhrases` property specifies custom phrases to use for each kind of help item. Its properties are optional and are indexed by the `HelpItem` enumeration, which lists the kinds of help items. They are summarized in the table below:
...
[custom error phrases]: #custom-error-phrases
[custom help phrases]: #custom-help-phrases
...
[parser custom phrases]: parser#custom-phrases
