...
The `AnsiMessage` class wraps a list of ANSI strings. It is the type used for help messages. This class provides a `wrap` method to get a traditional string. It accepts the following optional parameters:
...
When `FORCE_WIDTH` is not set, the default width used by this class is the `columns` property of the standard _output_ stream (i.e., `process.stdout{:ts}`). Therefore, this kind of message should be printed with `console.log{:ts}` or equivalent.
...
When `FORCE_WIDTH` is not set, the default width used by this class is the `columns` property of the standard _error_ stream (i.e., `process.stderr{:ts}`). Therefore, this kind of message should be printed with `console.error{:ts}` or equivalent.
...
Connective words cannot contain [inline styles] or line feeds.
