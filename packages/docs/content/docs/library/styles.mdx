...
ANSI-based messages can be configured via a `PartialMessageConfig` object that contains the optional properties described below.
...
The `connectives` property specifies connective words that are used to format some text elements that cannot be customized using phrases, such as option requirements, array element separators, and string quoting. It has the following optional settings, whose keys are enumerators from `ConnectiveWord`:
