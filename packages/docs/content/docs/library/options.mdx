...
Note that the library does not provide a deprecation mechanism for specific values within attributes, such as [option names], [parameter choices] or [data sources]. For that effect, you will have to explain it in the option [synopsis].
