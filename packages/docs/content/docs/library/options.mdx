...
It may be configured with a custom `toString` method to be displayed in [messages], e.g.:
...
The `useCommand` attribute is an opt-in feature that allows the next argument to be used as the name of a [subcommand] for which the help message should be generated.

For example, the following invocations would be equivalent:
...
If the data comes from the [environment], then the callback will receive an array with a single element as parameter, which may be _less_ than the expected count. To check if this is the case, you should inspect the `index` property of the [argument information], which should be `NaN{:ts}`.
...
It is useful in cases where the number of parameters is not known beforehand, and the callback wants to have control over where an [argument sequence] ends. Note that this is different from _unlimited_ parameters, which rely on the parser to detect them based on the occurrence of option names in the command line.
