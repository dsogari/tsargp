...
The `positional` attribute, if present, indicates that the option accepts _positional_ arguments. If set, then any argument not recognized as an option name will be considered positional.

If there are multiple positional options, their declaration order determines their relative position in the command line. Variadic options will take up all remaining positional arguments (up to a [trailing marker]).
...
> [!TIP]
>
> You can assign a unique [group] to positional options and place them _first_ in the option [definitions], in order to highlight them in the help message.

#### Trailing marker

The `marker` attribute, if present, indicates that the option accepts _trailing_ arguments. If set, then all arguments that appear beyond the marker will be considered trailing.

> [!WARNING]
>
> If there are multiple trailing markers, their usage will be _mutually exclusive_ in the command line.

> [!NOTE]
>
> The marker should _not_ be included in the list of [option names], although it is subject to the same restrictions.
