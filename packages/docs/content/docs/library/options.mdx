...
- A name can be the empty string. (Although this may look strange in help messages.)
...
> [!NOTE]
>
> Should not contain [inline styles] or line feeds. Any other Unicode character is allowed.
...
> [!NOTE]
>
> Overrides [example value] in usage statements. Should not contain line feeds.
...
> [!NOTE]
>
> Overrides [parameter name] in usage statements. Mutually exclusive with [example value]. Should not contain line feeds.
...
- `StyledString{:ts}` -- unstructured text data (e.g., a semantic version); or
