...
declare const values: Readonly<{
  ...
  strRegex: string;
  numRange: number;
  strChoice: 'one' | 'two' | undefined;
  numChoice: number | undefined;
  strArray: readonly ['one'] | readonly string[];
  numArray: readonly [1, 2] | readonly number[];
  strArrayLimit: readonly string[] | undefined;
  numArrayUnique: readonly number[] | undefined;
}>;
```

Notice that the properties in this object are `readonly{:ts}`. This is intentional: we believe that the parsed values should _not_ be modified, because they reflect the command line. Generally, you can do any necessary transformations inside the [parsing callback] and return arbitrary values, such as class instances, which should be enough for most purposes.
