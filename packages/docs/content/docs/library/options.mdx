...
The callback should return the parsed value (or a promise thereof). To throw an error from within it, we recommend instantiating the [error message] class: it accepts a custom phrase that will be formatted exactly like [error phrases].
