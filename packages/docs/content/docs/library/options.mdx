...
The `positional` attribute, if present, indicates that the option accepts _positional_ arguments. An argument is considered positional if it is not recognized as an option name or appears after the [trailing marker].
...
[trailing marker]: parser#trailing-marker
