...
The `names` attribute lists the option names as they may appear on the command line (e.g., `-h` or `--help`). Names must not contain whitespace or the equals sign `'='{:ts}`, since this character can be used as option-parameter separator. A name can also be `null{:ts}`, in which case the corresponding [name slot] will be skipped in the help message.
...
The `cluster` attribute, if present, specifies letters that can be used to combine options into a single command-line argument. This feature is also known as the [short-option] style, and can be enabled via the [`clusterPrefix`] parsing flag.
...
You can also specify a custom callback to evaluate requirements. It receives a single parameter which contains the parsed values, and should return a `boolean{:ts}` indicating whether the requirements were satisfied. It may perform any kind of verification, such as inspect parsed values, check the environment, etc.
...
An option is considered specified if it either appears on the command line or its data exists in the [environment].
...
In case the default value is not known beforehand, you can use a callback. It receives a single parameter containing parsed values (which you can inspect to determine the default), and should return the option value.
...
It receives different kinds of data depending on the option type, but its second parameter always contains the current [argument information], including the option's name, argument index and any previously parsed values.
...
The parser only reads _readily_ available data, i.e., it does not wait for _new_ input. This means that it can only extract data from redirected command outputs, not from interactive terminals. For the latter effect, you should use the [default callback] in conjunction with third-party prompting libraries.
...
It should return the list of completion words. If an error is thrown, it will be ignored and the default completion message will be thrown instead.
...
- the path of a JSON file from where a `version` field will be extracted
...
- an object containing the option definitions
- a callback that returns the option definitions (allowing the implementation of [recursive commands])
- the path of a JavaScript module exporting the option definitions (to be used with the [resolve callback] parsing flag)
...
The `clusterPrefix` attribute, if present, indicates whether the command accepts cluster arguments. It has the same semantics as its homonymous property in the [parsing flags], and must be used in conjunction with the [cluster letters] of the nested options.
...
The parse callback receives a `string[]{:ts}` as first parameter, which contains the option parameters. As a special case, this array may contain _all_ the remaining command-line arguments (see [parameter count] for details). It should return the option value.
...
[parsing flags]: parser#parsing-flags
