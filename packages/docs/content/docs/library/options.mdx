...
- A name can contain whitespace or even be an empty string. (However, this will look strange in help messages.)
...
An option is considered specified if either it appears on the command line or its data exists in the [environment].

</Callout>

#### Default value

The `default` attribute, if present, specifies a value to be used at the end of the parsing loop, in case the option is specified neither on the command line nor in the [environment].
...
The parser only reads _readily_ available data, without waiting for new input. This means that it can only extract data from redirected command outputs, not from interactive terminals. For the latter effect, you should use the [default callback] in conjunction with third-party prompting libraries.
...
The `choices` attribute, if present, specifies string values that the option accepts as parameter(s). Any parameter that does not match one of these values (while respecting the [case insensitive] attribute) will cause an error to be raised. They are also considered by the [completion algorithm] when a parameter is being completed.
...
The `mapping` attribute, if present, specifies a record that maps parameter values to option values. In this case, if a parameter matches one of the keys (while respecting the [case insensitive] attribute), it will get mapped to the corresponding value; otherwise it will be forwarded to the [parse callback], if specified.

#### Case insensitive

The `caseInsensitive` attribute, if present, indicates that either the list of [parameter choices] or the [parameter mapping] keys are to be evaluated in a case-insensitive manner. This includes parsing of option parameters and performing [word completion].
...
In the latter case, the path should generally be `'./package.json'{:ts}` and should be used in conjunction with the [resolve callback] parsing flag.
...
A callback allows the implementation of [recursive commands], whereas a module path allows a [dynamic import]. The latter case should be used in conjunction with the [resolve callback] parsing flag. In either case, it is only evaluated once the affected option is parsed from the command line.
...
The **flag** option accepts no parameter, but has a value that will be either the result of the [parse callback], if specified, or `true{:ts}`. The callback receives an empty string as its first parameter.
...
The **single** option accepts a single parameter. Its value can be either the result of the [parse callback], one of the [parameter choices], one of the [parameter mapping] values or the parameter itself (by default).
...
The **array** option accepts multiple (zero or more) parameters. Its value is an array whose elements can be either the result of the [parse callback], one of the [parameter choices], one of the [parameter mapping] values or the parameters themselves (by default).
...
The **function** option exists for the sole purpose of being configured with an expected parameter count, and is the only option that can accept between zero and one parameter. Its value will be either the result of the [parse callback], if specified, or the parsed values themselves.

The parse callback receives a `string[]{:ts}` as first parameter, which contains the option parameters, and should return the option value. As a special case, this array may contain _all_ the remaining command-line arguments (see [parameter count] for details).
...
In the special case of zero, the parse callback will receive all remaining command-line arguments. It should be used in conjunction with the [skip count] attribute. Note that altering the string array inside the callback would have _no_ effect on the original arguments, as it is just a copy.
...
If the data comes from the [environment], then the [parse callback] will receive an array with a _single_ element as parameter, which may be less than the expected count. You should inspect the `index` property of the [argument information], which should be `NaN{:ts}` in this case.

</Callout>

#### Skip count

The `skipCount` attribute indicates the number of remaining arguments to skip, after returning from the [parse callback]. This value is meant to be changed by the callback, and only works when the [parameter count] is zero.
...
[resolve callback]: parser#resolve-callback
