...
The callback may perform any kind of verification, such as inspect parsed values, check the environment, etc.
...
It may receive different kinds of data depending on the option type, but its second parameter always contains the current [argument information], including the option's name, argument index and any previously parsed values.
...
You may also check whether an option has been specified before it by comparing its value with `undefined{:ts}`. This works because default values are only set at the _end_ of the parsing loop.
...
It should return the list of completion words. If an error is thrown, it is ignored, and the default completion message will be thrown instead.
...
The `clusterPrefix` property indicates whether the command accepts cluster arguments. It has the same semantics as the homonymous property of the [parsing flags], and must be used in conjunction with the [cluster letters] of the nested options.
...
The `optionPrefix` property specifies the prefix for option names within the command. If set, then arguments starting with this prefix will be considered an option name, regardless of their position respective to other arguments in the command-line.
