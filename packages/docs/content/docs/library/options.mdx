...
<Callout type="info">
  ...
  argument may still be considered as an option filter when [enable option filter] is set.
</Callout>
...
<Callout type="default">
  This feature may be used in conjunction with [enable nested command]. For example, the invocation
  `cli --help cmd -f` would throw the help message of the `cmd` command, filtered by option `-f`.
</Callout>
...
<Callout type="info">
  When [word completion] is in effect, the parse callback will _not_ be called, since the completion
  will have taken place before it gets a chance to execute.
</Callout>
