...
The `positional` attribute, if present, indicates that the option accepts _positional_ arguments. The following values may be used:

- `true{:ts}` -- any argument not recognized as an option name will be considered positional
- `string{:ts}` -- all arguments beyond the positional marker will be considered positional

If there are multiple positional options, their declaration order determines their relative position in the command line. Variadic options will take up all remaining positional arguments (up to the positional marker, if any).
...
> [!NOTE]
>
> There may be _at most one_ option with a positional marker in the same [definition] level. Also, the marker should _not_ be included in the list of [option names], although it is subject to the same restrictions.
