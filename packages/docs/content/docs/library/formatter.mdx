...
The `optionFilter` property specifies a lists of patterns to select a subset of options. If not present, all options will be included respecting the order of their [definitions].
...
The first column contains the options' names separated by the value of the `optionSep` [connective word]. Names are listed in the order specified in the [option names] attribute.
...
The second column is rendered according to the [parameter rules].
...
```ansi
  demo.js [[38;2;160;100;64m-f[39m|[38;2;160;100;64m--no-flag[39m]
          ...
          [([35m-sa[39m|[35m--strArray[39m|[35m-S[39m) [90m[<param>...][39m|[35m-[39m]
          [([35m-na[39m|[35m--numArray[39m|[35m-N[39m) [90m[<param>...][39m]
...
- `exclude` -- whether the filter should exclude, or what to exclude (defaults to `false{:ts}`)
...
> [!NOTE]
>
> The `filter` and `exclude` properties match exactly. Options are listed in order of inclusion. The exception is an option with a [positional marker], which comes _last_ in the usage.
...
> [!NOTE]
>
> The `filter` and `exclude` properties match exactly. Groups are listed in order of inclusion.

## Layout settings

The layout of a [help column] applies to all help entries within the containing [groups section]. It can be configured via the settings described below.
...
The `slotIndent` setting is currently only available for the [names column]. It means that each name reserves a "slot" in the column, and the name will be aligned within that slot. (See [name slots] for more information.)
...
const layout: WithSectionGroups = {
  names: {
    slotIndent: 1, // assign a slot to each option name, with one space between slots
  },
  param: {
