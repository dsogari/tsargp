...
Both the **usage** and **groups** sections have a `filter` property with these optional properties:
...
> [!NOTE]
>
> Matches are exact. Exclusion properties have precedence over inclusion ones. Options and groups are listed in order of their [definitions]. The only exception is an option with a [positional marker], which comes _last_ in a usage statement.
