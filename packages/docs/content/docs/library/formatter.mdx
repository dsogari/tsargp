...
> [!NOTE]
>
> Matches are exact. Exclusion properties have precedence over inclusion ones. Options and groups are listed in order of their [definitions]. The only exception is an option with a [trailing marker], which comes _last_ in a usage statement.
...
> [!NOTE]
>
> There can be at most _one_ option with the [trailing marker] in a usage; otherwise an error will be raised.
