...
When the `responsive` property is set to `false{:ts}`, the terminal width is ignored and text wrapping enters into what we call [line-wise wrapping]. In this mode, lines are continued from one column to the next, allowing a column to "break" its lines independently of other columns.
...
[line-wise wrapping]: styles#line-wise-wrapping
