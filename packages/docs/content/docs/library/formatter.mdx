...
- `type` - tye type of section (required, can be `'text'{:ts}`, `'usage'{:ts}` or `'groups'{:ts}`)
- `heading` - the section heading (optional [text block], defaults to none)
- `content` - the section content (optional [text block], defaults to none)

<Callout type="info">

If the `content` is present and contains text, it will always include a _trailing_ line feed.
...
- `noSplit` - true to disable [text splitting] (defaults to `false{:ts}`)

### Text section

A text section can be used to write various kinds of content, such as introduction, instructions, afterword, copyright notice or external references. It contains the [common properties] and nothing else.

### Usage section

The [usage message] is a concise representation of a program's command line. It replaces the `content.text` property. Here is an example:
...
- `requires` - a mapping of options to required options (defaults to none)
...
A groups section is a collection of option groups and their help entries. The [option group] attribute is used as heading for each group, except for the _default_ group, which uses the `heading.text` property. The default group also uses the `content.text` property as a preface before listing its entries.

In addition to the [common properties], a groups section has the following optional properties:
...
- `noBreakFirst` - whether the first group heading should have no leading line feeds (defaults to `false{:ts}`)
