...
The `phrases` property specifies custom phrases to use for each kind of help item. Its properties are optional and are indexed by the `HelpItem` enumeration, which lists the kinds of help items. They are summarized in the table below:
