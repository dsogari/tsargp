...
> [!WARNING]
>
> Beware that if the section layout is not configured, using entry-specific settings may not have the desired effect. This is because the formatter does not _aggregate_ the layout settings of all help entries. Rather, it simply ensures that, if an option declares specific settings, they will override the section settings.
