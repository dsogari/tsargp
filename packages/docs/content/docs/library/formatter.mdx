...
The `progName` property is the [program name] to display in [usage sections]. If not present or empty, usage statements will contain no program name.
...
The **usage** and **groups** sections share these properties:

- `include` -- a list of options or groups to include (defaults to including all)
- `exclude` -- a list of options or groups to exclude (defaults to excluding none)
...
> [!NOTE]
>
> These properties match exactly. Options and groups are listed in order of inclusion. The exception is an option with a [positional marker], which comes _last_ in a usage statement.
...
In addition to the [common properties] and [filter properties], a **usage** section has the following optional properties:
...
- `compact` -- whether to keep alternatives compact (defaults to true)
...
The `exclusive` property of a **usage** section is reserved for mutual exclusivity. This feature is _not_ supported at the time. For that purpose, you have to create different usages.
...
In addition to the [common properties] and [filter properties], this type of section has the following optional properties:

- `names` -- the [layout settings] for the [names column]
- `param` -- the [layout settings] for the [parameter column]
- `descr` -- the [layout settings] for the [description column]
...
- `responsive` -- whether the overall layout is responsive (a.k.a., terminal-aware) (defaults to `true{:ts}`)

> [!TIP]
>
> The `useEnv` property can be used to create a dedicated section for environment variables.

When the `responsive` property is set to `false{:ts}`, the terminal width is ignored and text wrapping enters into what we call "line-wise" wrapping. In this mode, lines are continued from one column to the next, allowing a column to "break" its lines independently of other columns. For this to work, the length of the longest word in a column must not exceed the column's `maxWidth` property, if set. Otherwise, an error will be raised to alert the developer of this issue.

> [!NOTE]
>
> The reason why a non-responsive layout may raise an error is that we cannot split _words_, as they may have [inline styles], and those styles should not leak into subsequent columns.

## Layout settings

The layout of a [help column] applies to all help entries within the containing section. It can be configured via the settings described below.
...
[alignment setting]: #common-settings
