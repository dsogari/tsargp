...
- otherwise
  ...
  - and,
    - if the result does not contain angle brackets
    - then, enclose the result in angle brackets `'<>'{:ts}`
  - and,
    - if it requires [inline parameters], prepend an equals sign `'='{:ts}`
    - else, if it accepts multiple parameters, append an ellipsis `'...'{:ts}`
  - and,
    - if it can be specified without parameters and is not a nameless positional argument
    - then, enclose the result in square brackets `'[]'{:ts}`
