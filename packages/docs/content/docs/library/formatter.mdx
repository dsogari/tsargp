...
1. If the option is [niladic] and is not a [subcommand], stop here and render nothing.
2. Else, evaluate the following rules and combine the result with that of step 3:
   - if it contains an [example value], render this value
   - else, if it contains a [parameter name], render this name
   - else, render an empty string
3. If it accepts multiple parameters, append an ellipsis `'...'{:ts}`.
4. If the result from previous steps is empty, stop here and render nothing.
5. Else, if it requires [inline parameters], prepend an equals sign `'='{:ts}`.
6. If it can be supplied without parameters, enclose the result in square brackets `'[]'{:ts}`.

The result might be something like `=<param>`, `[<param>...]` or `[...]`.

> [!WARNING]
>
> Displaying the parameter of single-valued options is especially important in [usage sections].
...
```ansi
  demo.js [([38;2;160;100;64m-f[39m|[38;2;160;100;64m--no-flag[39m)]
          [[([35m-b[39m|[35m--boolean[39m)] ([35m-sc[39m|[35m--strChoice[39m) [90;32m'one'[90m[39m]
          ...
          [([35m-sa[39m|[35m--strArray[39m) [90m[...][39m]
          [([35m-na[39m|[35m--numArray[39m) [90m[...][39m]
