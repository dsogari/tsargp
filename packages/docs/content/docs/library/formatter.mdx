...
In the case of an inclusion filter, options are listed in the specified order. The exception is an option with a [positional] marker, which must come _last_ in the usage.
...
The `names.align` property supports an additional value `'slot'{:ts}`, meaning that each name receives a "slot" in the column, and the name is left-aligned within that slot. See [name slots] for more information. This value will be ignored if `param.align` is set to `'merge'{:ts}` or if `param.hidden{:ts}` is `true{:ts}` and `descr.align{:ts}` is set to `'merge'{:ts}` (see below).
