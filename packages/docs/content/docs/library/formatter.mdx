...
> [!NOTE]
>
> Matches are exact. Options and groups are listed in order of their [definitions]. The only exception is an option with a [positional marker], which comes _last_ in a usage statement.
