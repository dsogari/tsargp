...
The `optionFilter` property specifies an [option filter] to select a subset of options. If not present, all options will be included respecting the order of their [definitions].
...
> [!TIP]
>
> You can create a personalized footer including the repository URL through the `sectionFooter` function, which can be imported from `'tsargp/utility'{:ts}`.
