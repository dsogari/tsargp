...
The `names.align` property supports an additional value `'slot'{:ts}`, meaning that each name receives a "slot" in the column, and the name is left-aligned within that slot. See [name slots] for more information. This value is ignored if `param.align` is `'merge'{:ts}` (see below).
