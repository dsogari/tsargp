...
The `validator` module handles validation of option definitions. It contains a function that accept a set of option definitions and performs the validation. It may also produce error and warning messages, which can be customized via [message configuration].
