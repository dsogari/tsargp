...
- **Option not suppliable** -- It must be possible for an option to be [supplied], i.e., the latter must either have an [option name], [cluster letter], [data source], or accept [positional] or [trailing] arguments.
