...
- **Option not suppliable** -- An option must be able to be [supplied], i.e., it must either have an [option name] or [cluster letter], accept [positional] arguments or have an [environment] attribute.
