...
The `validator` module handles validation of option definitions. It contains a function that accepts a set of option definitions and performs the validation. It may also produce error and warning messages, which can be customized via [message configuration].
...
To validate a set of option definitions, you must call the `validate` function. It returns a promise that resolves to a `ValidationResult` object with the following property:
...
This function is _asynchronous_, so you must use `await{:ts}` in order to resolve the returned promise.
