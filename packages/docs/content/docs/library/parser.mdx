...
The `stdinSymbol` property specifies a symbol for the [standard input] to be displayed in [usage statements] produced by [help] options. If not present, the standard input will not appear in usage statements.

### Suggest names

The `suggestNames` property indicates whether similar option names should be suggested in case of an unknown option error. Alternatively, a percentage can be specified for the similarity threshold (e.g., `0.6{:ts}`).
...
3. If the option was [supplied] neither after a [trailing marker] nor as an [inline parameter], then, if either of the following holds true, the available [option names] are filtered by the prefix _comp_ and returned:
...
[trailing marker]: options#trailing-marker
