...
The `positionalMarker` attribute, if present, specifies the marker(s) to delimit positional arguments. If set, then all arguments that appear after/within the marker(s) will be considered positional.

> [!NOTE]
>
> The marker(s) should not conflict with [option names], although they are subject to the same restrictions.
