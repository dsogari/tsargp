...
- `position` -- The sequence number relative to [positional] arguments (1-based), or `NaN{:ts}` if it is either not positional or comes after a [trailing marker].
- `name` -- The option name. Can be either of:
  - the [supplied] option name
