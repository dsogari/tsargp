...
The `stdinSymbol` property specifies a symbol for the [standard input] to display in help messages produced by [help] options. If not present, the standard input will not appear in usage statements.
