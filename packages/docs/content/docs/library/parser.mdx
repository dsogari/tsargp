...
From the client code perspective, suggestions are the result of a [completion callback]. Each of which can be a plain `string{:ts}` or an object extending `CompletionSuggestion`. It will be injected with the above properties by the parser and overridden by client properties. We recommend extending the `Suggestion` type from [autocomplete-types].
