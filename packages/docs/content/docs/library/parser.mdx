...
It defaults to the _basename_ of the executing script, and can be suppressed by setting it to an empty string.
...
The `stdinSymbol` property specifies a symbol for the [standard input] to display in [usage statements] produced by [help] options. If not present, the standard input will not appear in usage statements.
...
An [option parameter] may be passed on the command line using the syntax `<name>=<param>`. The parameter can also be a series of values delimited with a [separator], in the case of [array] options.
