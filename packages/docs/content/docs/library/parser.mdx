...
- `position` -- The sequence number relative to [positional] arguments (1-based), or `NaN{:ts}` if it is either not positional or comes after the positional marker.
- `name` -- The option name. Can be either of:
  ...
  - an environment [data source], if the sequence comes from the [environment]
  - the positional marker, if the sequence comes after the marker
