...
The `similarity` property indicates the similarity threshold for option name suggestions in case of an unknown option error. Values are given in percentage (e.g., `0.6{:ts}`). Zero means disabled.
