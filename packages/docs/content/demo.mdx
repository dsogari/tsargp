...
Terminal emulation has limited capabilities. Copy, paste, history, coloring, and completion work, but not as well as in a real shell. For a smoother experience, you may want to try the demo locally:
...
<Callout type="default">
  We recommend opening the browser console with `F12`, to see the messages logged by sub-commands,
  as well as to play with text wrapping of help messages.
</Callout>
...
- check if values get appended when an array-valued option is specified repeatedly (e.g., `--numbersUnique 1 --numbersUnique 2`)
...
- check if the `hello` sub-command prints its arguments on the console
- check if the `hello` sub-command can have its own options (e.g., `hello -h`)
  - in this help message, see that a positional option can have no name
- check if the `hello` sub-command can be specified recursively (e.g., `hello a hello b c hello`)

### Text wrapping

With the browser console still open, slide the screen splitter until the terminal width changes, then run the help option again and:
