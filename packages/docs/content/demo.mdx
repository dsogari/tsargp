...
- check if a string parameter matches one the choices (e.g., try `-se A` and see that it fails)
- check if a number parameter matches the required range (e.g., try `-n 1` and see that it fails)
- check if a number parameter matches one the choices (e.g., try `-ne=0` and see that it fails)
