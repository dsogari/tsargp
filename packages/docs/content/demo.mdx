...
2. check if all arguments after the positional marker are recognized as positional (e.g., `-- -f`)
...
3. check if the `-b` option cannot be specified without required options (e.g. `-b yes`)
4. check if the `-b` option can be specified with required options (e.g. `-b yes -sc one`)
