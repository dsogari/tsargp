...
```ansi
Invalid parameter to [35m-sr[39m: [32m'A'[39m. Value must match the regex [31m/^\d+$/[39m.
```
...
```ansi
Invalid parameter to [35m-sc[39m: [32m'A'[39m. Value must be one of: [32m'one'[39m, [32m'two'[39m.
```
...
```ansi
Invalid parameter to [35m-nr[39m: [32m'-3'[39m. Value must be within the range [[33m-2[39m, [33mInfinity[39m].
```
...
```ansi
Invalid parameter to [35m-nc[39m: [32m'0'[39m. Value must be one of: [32m'1'[39m, [32m'2'[39m.
```
...
```ansi
Option [35m--strArrayLimit[39m has too many values: [33m4[39m. Should have at most [33m3[39m.
```
...
```ansi
Option [35m-sc[39m does not accept inline parameters.
```
...
```ansi
Option [35m-nc[39m requires an inline parameter.
```
...
```ansi
Option [35m-b[39m requires ([35m-sc[39m or [35m-sa[39m == [[32m'a'[39m, [32m'b'[39m]).
```
...
4. check if the `-f` option can be specified through an environment variable (e.g., `NO_FLAG= ...`)
...
```ansi
Option [35m-f[39m is deprecated and may be removed in future releases.
{
...
```ansi
Option [35m--no-flag[39m is deprecated and may be removed in future releases.
{
...
```ansi
Option [35mFLAG[39m is deprecated and may be removed in future releases.
{
  [0mflag[2m:[0m [0m[33mtrue[0m[0m[2m,[0m
  [90m...[0m
}
...
```ansi
Option [35mNO_FLAG[39m is deprecated and may be removed in future releases.
{
  [0mflag[2m:[0m [0m[33mfalse[0m[0m[2m,[0m
  [90m...[0m
}
...
```ansi
Unknown option [35m-s[39m. Similar names are: [35m-sr[39m, [35m-sc[39m, [35m-sa[39m.
```
